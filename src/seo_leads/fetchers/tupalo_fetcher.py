"""
Tupalo UK Directory Fetcher

Scrapes UK company data from Tupalo UK directory.
Tupalo is a social business directory with UK coverage.
"""
...
class TupaloUKFetcher(BaseDirectoryFetcher):
    """
    Tupalo UK directory scraper
    
    Tupalo UK (https://tupalo.co.uk) is a social business directory
    providing local business listings across the UK.
    """
    
    def __init__(self):
        super().__init__("Tupalo UK")
    
    def get_base_url(self) -> str:
        """Return the base URL for Tupalo UK"""
        return "https://tupalo.co.uk"
    
    def build_search_url(self, city: str, sector: Optional[str] = None, page: int = 1) -> str:
        """Build search URL for Tupalo UK city/sector combination"""
    ...
    def get_listing_selector(self) -> str:
        """Return CSS selector for business listings on Tupalo UK"""
        return '.place-item, .business-card, .listing-item, .venue-item'
    
    def parse_listing(self, listing_element, city: str) -> Optional[CompanyBasicInfo]:
        """Parse a single Tupalo UK business listing"""
        try:
            ...
            website_selectors = [
                ...
                'a[title*="website" i]',
                '.place-website a',
                'a[href^="http"]:not([href*="tupalo"])'  # External links
            ]
            ...
            for selector in website_selectors:
                ...
                if website_elem:
                    ...
                    if href and href.startswith('http') and 'tupalo' not in href:
                        website = href
                        break
            ...
            logger.debug(f"Error extracting company info from Tupalo UK listing: {e}")
            return None
    
    def has_next_page(self, page_soup: BeautifulSoup) -> bool:
        """Check if there are more pages available on Tupalo UK"""
...
async def fetch_tupalo_companies(cities: Optional[list] = None, 
                               sectors: Optional[list] = None) -> int:
    """Convenience function to fetch UK companies from Tupalo UK"""
    async with TupaloUKFetcher() as fetcher:
...
if __name__ == "__main__":
    ...
    async def test_fetcher():
        async with TupaloUKFetcher() as fetcher:
            ...
            print(f"Found {companies_found} companies from Tupalo UK")
